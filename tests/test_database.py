...
@mock.patch('chitanda.database.calculate_migrations_needed')
@mock.patch('chitanda.database.sys')
...
@mock.patch('chitanda.database._get_versions')
@mock.patch('chitanda.database._find_migrations')
...
@mock.patch('chitanda.database.Path')
def test_find_migrations(path):
    with CliRunner().isolated_filesystem():
        path.return_value.parent.__truediv__.return_value.glob = mock.Mock(
            return_value=[
                Path.cwd() / '0001.sql',
                Path.cwd() / '0002.sql',
                Path.cwd() / '0003.sql',
