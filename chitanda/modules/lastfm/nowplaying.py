...
from chitanda.util import args, auth_only, register
...
async def call(*, bot, listener, target, author, args, private, username):
    ...
    time_since = _calculate_time_since_played(response, author)
    tags = await _get_track_tags(track, album, artist)
    return _format_response(author, track, album, artist, tags, time_since)
